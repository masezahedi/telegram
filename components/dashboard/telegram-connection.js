...
const phoneSchema = z.object({
  phoneNumber: z.string()
    ...
    .regex(/^\+?[0-9]+$/, { message: 'شماره تلفن فقط می‌تواند شامل اعداد و علامت + باشد.' }),
});
...
const passwordSchema = z.object({
  password: z.string().min(1, { message: 'رمز عبور دو مرحله‌ای را وارد کنید.' }),
});
  ...
  const handleSendCode = async (data) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || 'خطا در ارسال کد. لطفاً دوباره تلاش کنید.');
      }
    } catch (error) {
  ...
  const handleVerifyCode = async (data) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        await updateUserSession(response.stringSession, phoneForm.getValues('phoneNumber'));
        ...
        toast.error(response.error || 'کد نامعتبر است. لطفاً دوباره تلاش کنید.');
      }
    } catch (error) {
  ...
  const handleVerify2FA = async (data) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || 'رمز عبور دو مرحله‌ای نامعتبر است. لطفاً دوباره تلاش کنید.');
      }
    } catch (error) {
  ...
  const updateUserSession = async (session, phoneNumber) => {
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || 'خطا در ذخیره اطلاعات. لطفاً دوباره تلاش کنید.');
      }
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || 'خطا در قطع اتصال. لطفاً دوباره تلاش کنید.');
      }
    } catch (error) {
      ...
      {connected ? (
        <div className="space-y-4">
          <Alert className="bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800">
            ...
            <AlertDescription>
              شما با موفقیت به تلگرام متصل شده‌اید و می‌توانید از تمامی امکانات سایت استفاده کنید.
            </AlertDescription>
          </Alert>
        ...
        <div className="space-y-6">
          ...
          {step === 3 && requires2FA && (
            <Form {...passwordForm}>
              <form onSubmit={passwordForm.handleSubmit(handleVerify2FA)} className="space-y-4">
                <FormField
