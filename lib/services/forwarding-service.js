const ForwardingService = {
    ...
    try {
      ...
      const serverUrl =
        process.env.NODE_ENV === "production"
          ? "http://localhost:1332" // Use internal network in production
          : "http://localhost:3001"; // Use local development server
  ...
  async updateServiceStatus(id, isActive) {
    try {
      ...
      const serverResponse = await fetch(`${serverUrl}${serverEndpoint}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
