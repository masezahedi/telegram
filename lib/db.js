...
async function openDb() {
  ...
  await db.exec(`
    ...
    -- START: جدول جدید برای پرداخت‌ها
    ...
    -- END: جدول جدید برای پرداخت‌ها
  `);

  // (بقیه کدهای مربوط به ALTER TABLE و ... که قبلا در فایل شما بود، در اینجا قرار می‌گیرد)
  // ...
  ...
  if (!userColumnNames.includes("telegram_id")) {
    try {
      await db.exec("ALTER TABLE users ADD COLUMN telegram_id INTEGER UNIQUE;");
      console.log("✅ Added column telegram_id to users table");
    } catch (error) {
      if (!error.message.includes("duplicate column name")) {
        console.error(
          "❌ Error adding column telegram_id to users table:",
          error
        );
      }
    }
