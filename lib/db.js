...
async function openDb() {
  ...
  await db.exec(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      telegram_id TEXT UNIQUE, // <<--- ستون جدید برای آی‌دی تلگرام
  ...
  if (!userColumnNames.includes("telegram_id")) {
    try {
      // Attempt to add with UNIQUE constraint directly.
      // SQLite allows multiple NULLs in a UNIQUE column.
      await db.exec(`ALTER TABLE users ADD COLUMN telegram_id TEXT UNIQUE;`);
      console.log(
        `✅ Added telegram_id column with UNIQUE constraint to users table`
      );
    } catch (error) {
      console.warn(
        `⚠️ Could not add telegram_id column with UNIQUE constraint directly. It might already exist, or other data might conflict. Error: ${error.message}`
      );
      // Fallback or specific error handling if needed, e.g., adding column then index separately
      // For most new setups or if no conflicting pre-existing data, TEXT UNIQUE should work.
      // If it fails due to existing data, manual migration for duplicates might be needed by the user.
    }
  }
  ...
  // Add new columns if they don't exist (forwarding_services)
  const requiredForwardingColumns = [
    {
      name: "type",
  ...
  for (const column of requiredForwardingColumns) {
    if (!columnNames.includes(column.name)) {
      try {
        ...
        console.error(
          `❌ Error adding column ${column.name} to forwarding_services:`,
          error
        );
      }
    }
  ...
  try {
    ...
    console.log("Admin user 'boyitnew@yahoo.com' ensured.");
  } catch (error) {
