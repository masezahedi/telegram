...
async function openDb() {
  ...
  if (!columns.some((col) => col.name === "history_direction")) {
    await db.exec(
      `ALTER TABLE forwarding_services ADD COLUMN history_direction TEXT DEFAULT 'newest';`
    );
  }

  if (!columns.some((col) => col.name === "start_from_id")) {
    await db.exec(
      `ALTER TABLE forwarding_services ADD COLUMN start_from_id TEXT;`
    );
  }

  if (!columns.some((col) => col.name === "copy_direction")) {
    await db.exec(
      `ALTER TABLE forwarding_services ADD COLUMN copy_direction TEXT DEFAULT 'before';`
    );
  }
