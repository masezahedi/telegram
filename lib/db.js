...
async function openDb() {
  ...
  // Check if prompt_template column exists in forwarding_services
  const columns = await db.all(`PRAGMA table_info(forwarding_services);`);
  const hasPromptTemplate = columns.some(
    (col) => col.name === "prompt_template"
  );

  if (!hasPromptTemplate) {
    await db.exec(
      `ALTER TABLE forwarding_services ADD COLUMN prompt_template TEXT DEFAULT '';`
    );
  }
